<template>
    ...
    <CRow class="justify-content-center" v-if="counter<=questionNumber">
        <CCol :md="14">
            <CCardBody>
                <CRow>
                    <CCol :sm="10">
                                    ...
                                    <div class="card-body">
                                        ...
                                        <CTable class="table">
                                            <CTableHead>
                                               ...
                                               <CButton v-if="counter!=1" @click="precedent()" class="btn2" type="submit" :disabled="load">precedent</CButton>
                                                <CButton @click="updateq()" class="btn2" type="submit" >modifier</CButton>
    ...
    <CRow class="justify-content-center" v-if="counter>questionNumber || questionNumber==0">
        <CCol :md="14">
            <CCardBody>
                <CRow>
                    <CCol :sm="10">
                                ...
                                <div class="card">
                                    <div class="card-body">
                                        ...
                                        <CTable class="table">
                                            ...
                                            <CButton v-if="counter!=1" @click="precedent()" class="btn4" type="submit" :disabled="load">precedent</CButton>
...
export default {
    ...
    methods : {
        ...
        updateq(){
           axios.put("http://localhost:8000/api/quiz-question/update/"+this.current,{content:this.questionContent})
           .then(res=>
           {
               for (let i=0 ; i<this.answers.length;i++){
               axios.put("http://localhost:8000/api/quiz-answer/update/"+this.answers[i].id,{content:this.answers[i].content,valid:this.answers[i].valid})
               .then(res=>{
...
this.load=true 
        ...
        axios.post("http://localhost:8000/api/quiz-question/"+this.testID,{content:this.question})
        .then(res=>{
            for(let i=0;i<answers.length;i++)
            {
                axios.post("http://localhost:8000/api/quiz-answer/test/"+this.testID+"/question/"+res.data.question.id,{content:answers[i].choix,valid:answers[i].radio})
                .then(()=>{
                    if(i==answers.length-1)
                    {
                        ...
                        this.questions=res.data
                        this.questionNumber=this.questions.length
                        this.current=this.questions[this.questionNumber-1].id+1
                        this.counter++
                        axios.get("http://localhost:8000/api/quiz-answer/test/"+this.testID+"/question/"+this.questions[0].id)
                        .then(res=>{
                        this.answers=res.data
                        console.log(this.current,this.questions,this.counter)
                        })
                        })
        ...
        terminer(){
            this.$router.push({path:"/icons/liste1"});
        }
    }
}
</script>
