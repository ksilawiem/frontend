<template>
    <div id="app">
        ...
        <form class="form">
            ...
            <div>
                <label for="phone">entreprise:</label>
                <input id="phone" type="text" class="form-control" v-model="enterprise"/>
...
this.enterprise=res.data.company
      });
    },
    methods:{
modifierprofil(){
  axios.put("http://localhost:8000/api/updateprofil/users/"+this.user.id,{firstName:this.nom,lastName:this.prenom,email:this.email,address:this.adresse,linkedin:this.linkedin,twitter:this.twitter,company:this.enterprise}).then(res=>{
      this.$router.push('/pages/profilrecruteur')
      })
},
    }
}
