...
export default {
    ...
    axios.get("http://localhost:8000/api/offre/"+this.$route.params.id).then((res) => {
      this.offre=res.data
    });
  },
  methods:{
      postuler()
   {
    axios.post("http://localhost:8000/api/postuler/user/"+this.user["id"]+"/offre/"+this.$route.params.id,{startedAt:"2008-11-11 13:23:44"})
    .then((res)=>{
    localStorage.setItem("post",res.data.Postuler.id)
     localStorage.setItem("OffreID",this.$route.params.id)
